...
def highest_score_from_words(words)
  ...
  final_winner = []
  min_lengths = 0
  lowest_word_length = 0
  highest_score = scores.max

  word_scores.each do |word_hash|
    if word_hash[:score] == highest_score
      winner_array << word_hash
    end
  end

  if winner_array.length < 1
    winner_array.each do |word_hash|
      if word_hash[:word].length == 10
        final_winner << word_hash
      end
    end

    final_winner.sort_by {|x| x[:word].length}
    return final_winner.first
  else
